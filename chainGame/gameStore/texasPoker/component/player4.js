  ...
  componentWillMount(){
    ...
    DeviceEventEmitter.addListener("updateUi",function(){
      ...
      for(let i=0;i<robotPlayers.length;i++){
        if(robotPlayers[i].seatNo == Number(player.seatNo)+3||Number(robotPlayers[i].seatNo)+6 == player.seatNo){
          _this.setState({        
            display:"flex",
  ...
  render() {
    return (
      <View style={styles.wrap}>
        ...
        <Chip wrap="small" text={this.state.chipValue} align="rightLeftCenter" style={{display:this.state.chipDisplay}}/>
